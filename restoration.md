...
**Before Step 0 — check [case-log.md](case-log.md) for this source or its series.** The log only helps a future session if it's actually read, not just written to. If this document (or an earlier volume of the same series — see Multi-volume section below) has prior entries, reuse their decided conventions (transliteration choices, register calls, page-offset, DPI) instead of re-deriving them from scratch.
